#!/usr/bin/env python3
"""
Comprehensive test to verify logging functionality across PyAgenity modules.
"""
...
def test_logging_across_modules():
    """Test that logging is working across different PyAgenity modules."""

    # Test main test logger
    logger = logging.getLogger(__name__)
    logger.info("=== PyAgenity Logging Comprehensive Test ===")
    ...
    node_logger = logging.getLogger("pyagenity.graph.node")
    node_logger.debug("Node module logger test")

    edge_logger = logging.getLogger("pyagenity.graph.edge")
    edge_logger.debug("Edge module logger test")

    # Test state module logging
    logger.info("Testing state module logging...")
    agent_state_logger = logging.getLogger("pyagenity.state.agent_state")
    agent_state_logger.info("AgentState module logger test")

    exec_state_logger = logging.getLogger("pyagenity.state.execution_state")
    ...
    di_logger = logging.getLogger("pyagenity.utils.dependency_injection")
    di_logger.info("DependencyInjection module logger test")

    callbacks_logger = logging.getLogger("pyagenity.utils.callbacks")
    ...
    base_cp_logger = logging.getLogger("pyagenity.checkpointer.base_checkpointer")
    base_cp_logger.info("BaseCheckpointer module logger test")

    mem_cp_logger = logging.getLogger("pyagenity.checkpointer.in_memory_checkpointer")
    ...
    graph_error_logger = logging.getLogger("pyagenity.exceptions.graph_error")
    graph_error_logger.warning("GraphError module logger test")

    node_error_logger = logging.getLogger("pyagenity.exceptions.node_error")
    node_error_logger.error("NodeError module logger test")

    # Test store module logging
    logger.info("Testing store module logging...")
    store_logger = logging.getLogger("pyagenity.store.base_store")
    ...
    parent_logger = logging.getLogger("pyagenity")
    parent_logger.info("Parent 'pyagenity' logger test")

    child_logger = logging.getLogger("pyagenity.graph")
    child_logger.info("Child 'pyagenity.graph' logger test")

    grandchild_logger = logging.getLogger("pyagenity.graph.compiled_graph")
    grandchild_logger.info("Grandchild 'pyagenity.graph.compiled_graph' logger test")

    logger.info("=== PyAgenity logging system verified successfully ===")
