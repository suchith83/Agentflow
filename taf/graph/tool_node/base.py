...
from taf.adapters.tools import ComposioAdapter
from taf.publisher.events import ContentType, Event, EventModel, EventType
from taf.publisher.publish import publish_event
from taf.state import AgentState, ErrorBlock, Message, ToolCallBlock, ToolResultBlock
from taf.state.message_block import RemoteToolCallBlock
...
class ToolNode(
    ...
    def __init__(
        ...
        Note:
            ...
            dependencies with: `pip install taf[mcp]`
        """
        logger.info("Initializing ToolNode with %d functions", len(list(functions)))

        if client is not None:
            # Read flags dynamically so tests can patch taf.graph.tool_node.HAS_*
            mod = sys.modules.get("taf.graph.tool_node")
            ...
            if not has_fastmcp or not has_mcp:
                raise ImportError(
                    ...
                    "Install with: pip install taf[mcp]"
                )
