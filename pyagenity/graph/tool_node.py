...
class ToolNode:
        ...
        for name, fn in self._funcs.items():
            ...
            for p_name, p in sig.parameters.items():
                ...
                if (
                    ...
                    and hasattr(p.default, "__class__")
                ):
                    try:
                        # Check if default value is an Inject instance
                        if "Inject" in str(type(p.default)):
                            logger.debug(
                                "Skipping injectable parameter '%s' with Inject syntax", p_name
                            )
                            continue
                    except Exception as e:
                        logger.debug(
                            "Could not determine if parameter '%s' uses Inject: %s", p_name, e
                        )
    ...
    async def _internal_execute(
        ...
        for param_name, param in sig.parameters.items():
            ...
            # Check if parameter uses Inject[...] syntax
            if (
                ...
                try:
                    if "Inject" in str(type(param.default)):
                        logger.debug("Skipping Inject parameter '%s' during execution", param_name)
                        continue
                except Exception as e:
                    logger.debug("Could not check Inject for parameter '%s': %s", param_name, e)
