...
# from pyagenity.store.qdrant_store import EmbeddingService, OpenAIEmbeddingService
...
# @pytest.fixture
# async def qdrant_store(mock_embedding_service, mock_qdrant_client):
...
#     return Message.from_text("Hello, this is a test message", role="user")
...
#     def test_openai_embedding_service_without_openai(self):
#         """Test OpenAIEmbeddingService when OpenAI is not installed."""
#         with patch("pyagenity.store.qdrant_store.openai", None):
...
#         args, _ = mock_qdrant_client.upsert.call_args
#         assert len(args[1]) == 3  # points argument
...
#         user_id, agent_id, collection = qdrant_store._extract_config_values(config)
