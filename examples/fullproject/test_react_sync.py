...
class TestToolNode:
    ...
    def test_tool_node_execution(self):
        ...
        assert len(result_state.context) > len(state.context)
        # Last message should be a tool result
        last_message = result_state.context[-1]
...
class TestGraphConstruction:
    ...
    def test_graph_edges(self):
        ...
        assert "TOOL" in graph.edges
        assert "MAIN" in graph.edges["TOOL"]
    ...
    def test_graph_conditional_edges(self):
        ...
        assert "MAIN" in graph.conditional_edges
        conditional_edge = graph.conditional_edges["MAIN"]

        # Check that it routes to TOOL or END
        assert callable(conditional_edge.condition)
        assert "TOOL" in conditional_edge.path_map.values()
