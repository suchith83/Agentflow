    ...
    async def _chat_with_store_memory(self, state: MemoryAgentState) -> MemoryAgentState:
        ...
        try:
            ...
            memory_results = await self.store.asearch(
                ...
                memory_type="episodic"  # Filter by memory type (now using episodic for compatibility)
            )
...
Show that you remember previous topics and user preferences."""
        ...
        try:
            ...
            await self.store.astore_message(
                message=user_msg,
                user_id=user_id,
                memory_type="episodic",  # Use episodic for compatibility
                category="chat",
                additional_metadata={
    ...
    async def get_memory_stats(self, user_id: str) -> Dict[str, Any]:
        """Get memory statistics using store's stats method."""
        try:
            stats = await self.store.aget_stats(user_id=user_id)
            return stats
        except Exception as e:
    ...
    async def clear_user_memories(self, user_id: str) -> int:
        """Clear all memories for a user using store's delete_all method."""
        try:
            deleted_count = await self.store.adelete_all(user_id=user_id)
