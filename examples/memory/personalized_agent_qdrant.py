...
from pyagenity.utils.constants import START, END
...
class PersonalizedAgent:
        ...
        try:
            ...
            if 'results' in memories:
                for memory_item in memories['results']:
                    ...
                    if any(word in memory_text.lower() for word in ['prefer', 'like', 'favorite', 'enjoy']):
    ...
    async def _personalized_response_node(self, state: PersonalizedAgentState) -> PersonalizedAgentState:
        ...
        if state.user_profile.get("preferences"):
            preferences_context = "\nYour preferences: " + "; ".join(state.user_profile["preferences"].values())
...
Current conversation:"""
        ...
        try:
            ...
            ai_message = Message.text_message(
    ...
    async def chat(self, user_input: str, user_id: str, session_id: str = None) -> str:
        ...
        return result.messages[-1].content if result.messages else "I couldn't process your request."
